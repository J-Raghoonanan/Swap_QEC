"""
Streaming Purification Quantum Error Correction - Corrected Implementation

This module implements the streaming purification protocol with:
- Amplitude amplification followed by measurement-based post-selection
- Random initial states using Haar measure
- Separate pipelines for depolarizing and Pauli errors
...
class PurityParameterState:
    ...
    noisy_state_vector: np.ndarray  # Track actual noisy state
...
class ProtocolParameters:
    ...
    max_measurement_retries: int = 100  # Max retries for failed measurements
    measurement_noise_level: float = 1e-8
    pauli_approximation_method: str = "coherence_weighted"
    use_random_initial_states: bool = True
...
@dataclass
class AmplificationMeasurementResult:
    """Result of amplitude amplification followed by measurement."""
    initial_success_probability: float
    amplified_success_probability: float
    amplification_iterations: int
    measurement_successful: bool  # True if ancilla measured as |0⟩
    amplitude_evolution: np.ndarray
    gate_count: int
    total_retries: int  # Number of times we had to restart due to measurement failure
...
# ============================================================================
# NOISE MODELS WITH RANDOM STATES (unchanged)
# ============================================================================
...
class DepolarizingNoise(NoiseModel):
    ...
    def apply_noise(self, pure_state: np.ndarray) -> PurityParameterState:
        ...
        return PurityParameterState(purity, self.dimension, pure_state, pure_state)
...
class PauliNoise(NoiseModel):
    ...
    def apply_noise(self, pure_state: np.ndarray) -> BlochVectorState:
        ...
        return BlochVectorState(noisy_bloch, target_bloch, pure_state, coherence_params)
...
# ============================================================================
# SWAP TEST ANALYZERS WITH AMPLITUDE AMPLIFICATION + MEASUREMENT
# ============================================================================

class DepolarizingSwapTestAnalyzer:
    """Swap test analysis for depolarizing noise using amplitude amplification + measurement."""
    ...
    def compute_output_state(self, input_state: PurityParameterState) -> PurityParameterState:
        ...
        return PurityParameterState(output_purity, d, input_state.target_vector, input_state.noisy_state_vector)
    
    def simulate_amplification_and_measurement(self, initial_success_prob: float) -> AmplificationMeasurementResult:
        """
        Simulate amplitude amplification followed by measurement with potential retries.
        
        Process:
        1. Apply amplitude amplification to boost success probability
        2. Measure ancilla qubit
        3. If measurement yields |1⟩, discard and retry entire process
        4. If measurement yields |0⟩, success
        """
        if initial_success_prob >= 1.0:
            return AmplificationMeasurementResult(1.0, 1.0, 0, True, np.array([1.0]), 4, 0)
        ...
        amplified_prob = np.sin((2*optimal_iterations + 1) * theta / 2)**2
        
        # Track amplitude evolution during amplification
        ...
        # Now attempt measurement with potential retries
        total_retries = 0
        gate_count = 4 + 4 * optimal_iterations  # Base swap test + amplification rounds
        
        while total_retries < self.params.max_measurement_retries:
            # Simulate measurement of ancilla with amplified probability
            noisy_amplified_prob = amplified_prob + np.random.normal(0, self.params.measurement_noise_level)
            noisy_amplified_prob = np.clip(noisy_amplified_prob, 0, 1)
            
            measurement_success = np.random.random() < noisy_amplified_prob
            
            if measurement_success:
                # Success! Ancilla measured as |0⟩
                return AmplificationMeasurementResult(
                    initial_success_prob, amplified_prob, optimal_iterations, 
                    True, amplitude_evolution, gate_count, total_retries
                )
            else:
                # Failure: Ancilla measured as |1⟩, need to retry entire process
                total_retries += 1
                gate_count += 4 + 4 * optimal_iterations  # Full process repeated
        
        # If we reach here, we've exceeded max retries
        logger.warning(f"Amplification+measurement failed after {total_retries} retries with amplified prob {amplified_prob:.3f}")
        
        # Return failure (this will cause the purification to fail)
        return AmplificationMeasurementResult(
            initial_success_prob, amplified_prob, optimal_iterations,
            False, amplitude_evolution, gate_count, total_retries
        )

class PauliSwapTestAnalyzer:
    """Swap test analysis for Pauli errors with amplitude amplification + measurement."""
    ...
    def compute_output_state(self, input_state: BlochVectorState) -> BlochVectorState:
        ...
        # Add quantum noise
        noise = np.random.normal(0, self.params.measurement_noise_level, 3)
        ...
        return BlochVectorState(restored_bloch, target_bloch, input_state.target_pure_state, updated_coherence)
    
    def simulate_amplification_and_measurement(self, initial_success_prob: float) -> AmplificationMeasurementResult:
        """Simulate amplitude amplification + measurement for Pauli case."""
        if initial_success_prob >= 1.0:
            return AmplificationMeasurementResult(1.0, 1.0, 0, True, np.array([1.0]), 4, 0)
        ...
        # Compute amplified success probability
        amplified_prob = np.sin((2*optimal_iterations + 1) * theta / 2)**2
        amplified_prob = min(1.0, amplified_prob + 
                           np.random.normal(0, self.params.measurement_noise_level))
...
# ============================================================================
# THRESHOLD ANALYSIS ENGINE (unchanged)
# ============================================================================
...
# ============================================================================
# PURIFICATION ENGINES WITH AMPLITUDE AMPLIFICATION + MEASUREMENT
# ============================================================================

class DepolarizingPurificationEngine:
    """Recursive purification engine for depolarizing noise with amplitude amplification + measurement."""
    ...
    def execute_purification(self, initial_state: PurityParameterState) -> PurificationResult:
        """Execute recursive purification with amplitude amplification + measurement."""
        levels = self.params.purification_levels
        fidelity_evolution = np.zeros(levels + 1)
        error_evolution = np.zeros(levels + 1)
        all_amplification_measurement_results = []
        ...
        for level in range(levels):
            ...
            # Perform swap test with amplitude amplification + measurement
            success_prob = self.analyzer.calculate_success_probability(current_state)
            amp_measurement_result = self.analyzer.simulate_amplification_and_measurement(success_prob)
            
            if amp_measurement_result.measurement_successful:
                # Measurement yielded |0⟩, apply purification
                current_state = self.analyzer.compute_output_state(current_state)
            else:
                logger.error(f"Amplitude amplification + measurement failed completely at level {level}")
                raise RuntimeError(f"Amplitude amplification + measurement failed completely at level {level}")
            
            all_amplification_measurement_results.append(amp_measurement_result)
            total_gates += amp_measurement_result.gate_count
            total_amplification_iterations += amp_measurement_result.amplification_iterations
            total_measurement_retries += amp_measurement_result.total_retries
        ...
        return PurificationResult(
            ...
            amplification_measurement_results=all_amplification_measurement_results,
            memory_analysis=memory_analysis,
            total_amplification_iterations=total_amplification_iterations,
            total_gate_complexity=total_gates,
            total_measurement_retries=total_measurement_retries,
...
class PauliPurificationEngine:
    """Recursive purification engine for Pauli errors with amplitude amplification + measurement."""
    ...
    def execute_purification(self, initial_state: BlochVectorState) -> PurificationResult:
        """Execute recursive purification for Pauli errors with amplitude amplification + measurement."""
        levels = self.params.purification_levels
        fidelity_evolution = np.zeros(levels + 1)
        error_evolution = np.zeros(levels + 1)
        all_amplification_measurement_results = []
...
# ============================================================================
# ENHANCED PROTOCOL INTERFACE (updated with corrected flow)
# ============================================================================
...
class StreamingPurificationProtocol:
    """Enhanced protocol with amplitude amplification + measurement and separate pipelines."""
    ...
    def run_single_purification(self, dimension: int, noise_type: NoiseType, 
                              ...
                              pauli_approximation_method: str = "coherence_weighted",
                              use_random_initial_states: bool = True) -> PurificationResult:
        """Run single purification with amplitude amplification + measurement."""
        ...
        if noise_type == NoiseType.DEPOLARIZING:
            ...
            params = ProtocolParameters(
                ...
                use_random_initial_states=use_random_initial_states
            )
        else:
        ...
        # Create target state - RANDOM or fixed
        if use_random_initial_states:
            target_vector = generate_random_pure_state(params.dimension)
            logger.info(f"Generated random target state with dimension {params.dimension}")
        else:
            target_vector = np.zeros(params.dimension, dtype=complex)
            target_vector[0] = 1.0  # |0⟩ state
            logger.info(f"Using fixed |0⟩ target state")
        
        # Route to appropriate pipeline - SEPARATE PIPELINES MAINTAINED
        if noise_type == NoiseType.DEPOLARIZING:
            ...
            for noise_strength in tqdm(noise_range, desc="Threshold sweep"):
                try:
                    result = self.run_single_purification(
                        ...
                        purification_levels=purification_levels,
                        total_input_states=total_input_states,
                        use_random_initial_states=True
                    )
