...
from .old_configs import RunSpec, NoiseMode
from .old_state_factory import build_target
from src.simulation.OLD.old_noise_engine import build_noisy_copy, sample_error_pattern, ErrorPattern
from src.simulation.OLD.old_amplified_swap import purify_two_from_density
...
def run_streaming(spec: RunSpec) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Build a prototype noisy copy for iid_p mode (reusable)
    ...
    if rho_init is None:
        # Build one sample copy for baseline in exact_k mode
        logger.info("Building sample noisy copy for baseline metrics (exact_k mode)")
    ...
    if M == 1:
        ...
        logger.info(f"  Bloch vector magnitude: |r⃗|={r_init:.6f} (theory: 1-δ={1-spec.noise.delta:.6f})")
    ...
    # For exact_k mode: track error patterns at level 0
    level0_patterns: Dict[int, ErrorPattern] = {}

    steps_rows: List[Dict] = []
    merge_counter = 0

    def _log_step(depth: int, rho_out: DensityMatrix, meta: Dict, inputs_used: int):
    ...
    def _get_new_copy_dm(i: int) -> DensityMatrix:
        ...
        # exact_k: sample new pattern per copy (will be shared at merge time)
        ...
        # Store pattern for potential reuse if this becomes part of a level-0 merge
    ...
    for i in range(spec.N):
        ...
        carry_dm = rho_new
        carry_count = 1
        carry_index = i  # Track which copy index this represents
    ...
    finals_row = {
        ...
        "twirling_enabled": spec._should_apply_twirling(),
    }
...
def run_and_save(spec: RunSpec) -> Tuple[Path, Path]:
    ...
    steps_path = out_dir / "steps_circuit_dephasing_v2.csv"
    finals_path = out_dir / "finals_circuit_dephasing_v2.csv"
