...
def _apply_local_deterministic_twirled_noise(
    ...
    *,
    ...
    NEW in subsetTwirling: This function now delegates to apply_noise_to_density_matrix
    which implements subset twirling based on spec.twirling.subset_fraction.

    If twirling_active and noise is dephase_z:
      Implements Clifford-averaged noise using a SUBSET of {I,H,SH}^⊗M:
        rho_out = (1/K) sum_{C in subset} C† Z_p( C rho C† ) C
      where K is determined by subset_fraction and subset_mode.

    Otherwise:
      Applies the raw noise channel once.
    """
    # Delegate to apply_noise_to_density_matrix which handles subset twirling
    return apply_noise_to_density_matrix(
        rho,
        spec.noise,
...
def run_and_save(spec: RunSpec) -> Tuple[Path, Path]:
    ...
    # Construct filename with subset twirling indicator
    suffix = spec.noise.noise_type.value
    
    # Add subset indicator to filename if using partial twirling
    twirl_suffix = ""
